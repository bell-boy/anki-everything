...
chrome.storage.local.get("AECardInfo").then(async ({AECardInfo : {sourceText, numCards}}) => {
  ...
  const cardObject : PublishCardsRequest = JSON.parse(completion.choices[0].message.content!) as PublishCardsRequest;

  cardObject.cards.map((cardText) => {
